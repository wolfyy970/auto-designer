...
3. **Resource loader + session factories** — `packages/auto-designer-pi/src/resource-loader.ts` filters skills by session-type tags (`SessionScopedResourceLoader`); `packages/auto-designer-pi/src/host.ts` exposes `createDesignSession` / `createEvaluationSession` / `createIncubationSession` / `createInputsGenSession` / `createDesignSystemSession`. Each wires the right tag set + extension factories.
...
**System message** = body from `packages/auto-designer-pi/prompts/_designer-system.md` (resolved via `loadDesignerSystemPrompt()`) + `Current date` and `Current working directory` appended by Pi's `buildSystemPrompt`. **User message** = hypothesis/spec + the relevant per-task guidance body inlined by the route (e.g. `gen-hypotheses.md` for incubation, `revise.md` for revision rounds). Skills are **not** copied into the VFS; the agent loads `SKILL.md` via `use_skill` for the three real skills and reads optional text resources via `list_skill_resources` / `read_skill_resource`. The Incubator's user message is assembled deterministically: `buildInternalContext(spec)` in [src/lib/prompts/internal-context.ts](src/lib/prompts/internal-context.ts) wraps each spec section verbatim in XML tags. There is no LLM-synthesised "internal context" or "design specification" intermediate.
