...
2. **Tool builders + designer extension** — `packages/auto-designer-pi/src/tools/*.ts` and `packages/auto-designer-pi/src/extension/*.ts` build Pi-native `ToolDefinition`s for `read` / `write` / `edit` / `ls` / `find` / `grep` / `bash` plus the app-level `todo_write`, `validate_js`, and `validate_html`. Pi's own `use_skill` / `list_skill_resources` / `read_skill_resource` come from the SDK's resource loader and read from the package's `skills/` directory. The wrapped `edit` tool retries once on "could not find" via `tools/edit-match-cascade.ts`.
3. **Resource loader + session factories** — `packages/auto-designer-pi/src/resource-loader.ts` filters skills by session-type tags (`SessionScopedResourceLoader`); `packages/auto-designer-pi/src/host.ts` exposes `createDesignSession` / `createEvaluationSession` / `createIncubationSession` / `createInputsGenSession` / `createDesignSystemSession` / `createInternalContextSession`. Each wires the right tag set + extension factories.
4. **Bundled prompt content** — Three real skills (`accessibility`, `design-generation`, `design-quality`) under `packages/auto-designer-pi/skills/<key>/SKILL.md`, plus per-task prompt templates under `packages/auto-designer-pi/prompts/<name>.md` (including the designer system prompt `_designer-system.md`). `loadDesignerSystemPrompt()` and `loadPackagePromptBody(filename)` expose those bodies for host code that inlines guidance (e.g. `incubate.ts` injects `gen-hypotheses.md`).
...
**System message** = body from `packages/auto-designer-pi/prompts/_designer-system.md` (resolved via `loadDesignerSystemPrompt()`) + `Current date` and `Current working directory` appended by Pi's `buildSystemPrompt`. **User message** = hypothesis/spec + the relevant per-task guidance body inlined by the route (e.g. `gen-hypotheses.md` for incubation, `gen-internal-context.md` for internal-context, `revise.md` for revision rounds). Skills are **not** copied into the VFS; the agent loads `SKILL.md` via `use_skill` for the three real skills and reads optional text resources via `list_skill_resources` / `read_skill_resource`.
...
| `use_skill` / `list_skill_resources` / `read_skill_resource` | App | Pi SDK (`@mariozechner/pi-coding-agent`) over the package's `skills/` directory | `name` (string, **required**); `read_skill_resource` also takes `path` (string) | Native Pi skill loader. `use_skill` loads `SKILL.md` for one of the three real skills (`accessibility`, `design-generation`, `design-quality`); resource tools list/read sibling text files. Scripts are readable source only. |
...
**Scope:** Virtual file tools and just-bash apply during **design** **agentic Pi sessions** (initial build + each revision round in `agentic-orchestrator.ts`). Incubation, inputs-gen, design-system extraction, internal-context, and evaluator steps run through the same package-backed pipeline with **session-scoped** skill catalogs; **meta-harness** hits the same API routes. Keep `packages/auto-designer-pi/prompts/_designer-system.md`, `server/lib/prompt-templates.ts` placeholders, and tool descriptions aligned with this doc.
...
| `lib/pi-stream-budget.ts`                  | Per-turn `max_tokens` heuristic from prompt-token estimates; named constants for per-image and tokenizer-fudge multipliers                                                                                                                              |
...
| `lib/inline-guidance.ts`                   | `inlineGuidance(key, tag)` — single source of truth for the "load body → wrap in `<tag>…</tag>` → splice into agent user prompt" convention used by incubate, internal-context, inputs-generate, and design-system routes                              |
...
| `lib/build-agentic-system-context.ts`      | Composes the agentic system context: `loadDesignerSystemPrompt()` body + `skillCatalog` entries discovered from the package's `skills/` dir + empty `sandboxSeedFiles` (orchestrator may add caller seeds only); skills are **not** copied into the VFS |
| `lib/skill-discovery.ts`                   | Reads `packages/auto-designer-pi/skills/*/SKILL.md` for the `skills_loaded` SSE catalog only; filters by session-type tags. Pi's resource loader inside the package owns `use_skill` / `list_skill_resources` / `read_skill_resource`.                  |
...
`server/routes/generate.ts` delegates to `server/services/agentic-orchestrator.ts` → `runAgenticWithEvaluation`. The agentic system prompt body is `packages/auto-designer-pi/prompts/_designer-system.md`, loaded via `loadDesignerSystemPrompt()`; the skill catalog (the package's three real skills) reaches the model through Pi's native `use_skill` tool, not appended to the system prompt. Skills are not copied into the just-bash virtual filesystem — the agent loads `SKILL.md` via `use_skill` and optional text resources via the package's resource tools.
...
**Why the server resolves prompts from the package.** Prompt bodies ship with `@auto-designer/pi` under `packages/auto-designer-pi/{skills,prompts}/`. `server/lib/prompt-resolution.ts` resolves them per request through `loadDesignerSystemPrompt()` and `loadPackagePromptBody()`. The SPA sends workspace/spec payloads and model settings; prompt text is never client-editable.

**Why `.prompt-versions/` and `_versions/` exist.** Tunable copies (package skills, package prompt templates, `src/lib/rubric-weights.json`) get **shadow snapshots** before automated overwrites: [meta-harness/version-store.ts](meta-harness/version-store.ts) implements `snapshotBeforeWrite` and `snapAll`, wired from meta-harness and `scripts/snap.ts`. Package skill snapshots live under `packages/auto-designer-pi/skills/<key>/_versions/`; prompt-template snapshots under `packages/auto-designer-pi/prompts/_versions/<name>/` (single shared dir, one subdir per template); rubric JSON stays under `.prompt-versions/snapshots/`; `manifest.jsonl` is the append-only log. `_versions/` is not read by skill discovery (history stays out of the agent catalog). Operators run `pnpm snap` (changed files only) or rely on the pre-commit hook — see [USER_GUIDE.md § Version history](USER_GUIDE.md#version-history). Meta-harness write paths: [meta-harness/VERSIONING.md](meta-harness/VERSIONING.md).
...
The `meta-harness/` package is a repo-local **Node CLI** (`pnpm meta-harness`) that exercises the same `/api/*` flows as the canvas (incubate, hypothesis generate + eval, inputs-generate) using JSON fixtures, an OpenRouter-based **proposer**, and gitignored `meta-harness/history/session-<mode>-*/` artifacts (folder name includes `incubate` | `e2e` | `design` | `inputs` before the timestamp; per-session `PROMOTION_REPORT.md` at the session root). **Four modes:** `--mode=incubate` (hypothesis rubric), `--mode=inputs` (inputs auto-fill quality rubric), `--mode=design` (agentic build + eval), `--mode=e2e` (inputs-gen → incubate → build → eval). Before the run loop it can **preflight** the most recent completed session: compare `skills-snapshot/` to `packages/auto-designer-pi/skills/` and `candidate-*/rubric-weights.json` to `src/lib/rubric-weights.json`, showing unified diffs in Ink (or stdout in `--plain`) unless `--skip-promotion-check` or `--improve`. In a TTY, `P` applies drift to the package skills and `src/lib/rubric-weights.json`. The designer system prompt lives in `packages/auto-designer-pi/prompts/_designer-system.md` — promote it via normal git, not harness `P`. `--promote` is preflight-only (plus health); `--plain --promote` prints diffs without auto-apply. It is not bundled in the SPA build. Operator runbook: [meta-harness/README.md](meta-harness/README.md) and [meta-harness/META_HARNESS_OUTER_LOOP.md](meta-harness/META_HARNESS_OUTER_LOOP.md).
