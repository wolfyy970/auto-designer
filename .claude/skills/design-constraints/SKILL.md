...
| 3 | **Target surface(s) declared with justification when open** | Specific surface(s), surface mix, or an explicit `open` marker WITH a one-clause justification for why surface is part of the exploration space (e.g., "the audience uses multiple channels"; "the channel choice is itself a hypothesis"). | No surface declaration; or implicit/inferred surface without explicit statement. *Also*: `open` declaration without justification — surface choice marked `open` because committing felt hard, not because surface is genuinely part of what hypotheses propose. |
| 4 | **Audience reach realities named where they filter viability for THIS brief** | Audience characteristics that constrain reach — phrased as exploration-space filters; the axes named are the ones that actually filter direction-fit for this brief's specific audience. | Execution-layer accessibility criteria (contrast ratios, ARIA, keyboard nav rules) framed as audience-reach constraints. *Also*: rote application of a default audience-reach checklist (LEP / disability / older adults / low-bandwidth / low-literacy) regardless of whether each axis is load-bearing for this brief's audience. |
...
| 11 | **Empirical claims sourced or marked Inferred; framework citations carry their claims** | Sourced claims have citations; extensions from sourced findings are marked `(Inferred)`; qualitative magnitude language used where no source is available. Where a cognitive framework (Cognitive Load Theory, Yerkes-Dodson, Mullainathan-Shafir scarcity, etc.) is cited, the specific mechanism / number / behavior asserted afterward is actually in the cited work. | Statistics without citation; framework-translated specifics asserted as if both the framework name and the number are cited. *Also*: framework citation as topical decoration — the framework name is invoked at the head of a claim whose specifics aren't actually in the cited work (the framework name is doing rhetorical lift it doesn't carry). If the specific isn't in the framework, mark it `(Inferred)` and don't attribute. |
...
5. **Template-fatigue across the corpus.** A second-order failure that emerges after the first pass of rubric cleanup: rubric-compliant docs that nonetheless share the same five-paragraph skeleton, the same parenthetical hedges ("rendering-layer specifics live in execution work, not here"), the same framework citations (Cognitive Load Theory, Yerkes-Dodson, Mullainathan-Shafir scarcity) regardless of whether the framework's specific mechanism actually filters viability for the brief at hand, and the same audience-reach checklist (LEP / disability / older adults / low-literacy / low-bandwidth) regardless of the brief's audience composition. The rubric tests for *layer correctness* (strategic vs execution); domain-specific authorship is a separate discipline the rubric does not enforce directly. When you find yourself reaching for known-passing patterns, ask: would removing this sentence change what this brief's specific domain implies? If no, the sentence is template-fill, not constraint. Two constraints docs from different domains should not share an opening paragraph beyond the surface-declaration shape.
