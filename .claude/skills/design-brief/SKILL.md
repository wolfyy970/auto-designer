...
- **SCUC four-section** *(this skill's canonical)*: one short paragraph each for Situation, Complication, Users (target user(s) named at specificity), and Cost. Use **H2 subheadings on their own line** in the brief body — `## Situation`, `## Complication`, `## Users`, `## Cost`, each followed by a blank line and one paragraph of body — so any downstream reader (human, agent, or video viewer) can see the SCUC bones at a glance and extract each slot unambiguously. The brief title stays as a single H1 above; SCUC slots are the four H2 sections beneath it. Most explicit form of the structure; useful when authors want to verify each component is present and slot-by-slot well-formed.
