...
- **SCUC four-paragraph** *(this skill's canonical)*: one short paragraph each for Situation, Complication, Users (target user(s) named at specificity), and Cost. Use **bold inline labels** at the start of each paragraph — `**Situation.** …`, `**Complication.** …`, `**Users.** …`, `**Cost.** …`, one per paragraph — so any downstream reader (human or agent) can extract each slot at a glance. Labels are inline text emphasis, not block headings; the brief stays a four-paragraph problem statement and the canvas brief node renders it as prose, not as a sectioned document. Most explicit form of the structure; useful when authors want to verify each component is present and slot-by-slot well-formed.
