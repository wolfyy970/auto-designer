...
2. **Design instructions body** — Prompt key [`designer-agent-instructions`](src/lib/prompts/defaults.ts): bundled file [`design-agent-instructions.md`](packages/auto-designer-pi/prompts/design-agent-instructions.md) (YAML frontmatter stripped via [`getPromptBody()`](server/lib/prompt-resolution.ts)). Same tagging style as other package prompts (`<how_to_think>`, `<what_to_write>`, `<quality_bar>`, `<final_report>`).
...
| Incubation | `hypotheses-generator-system` / `gen-hypotheses.md` | Axes + strategies: infers product scope when unstated; frames each hypothesis as **MVP market-fit probe** with **`measurements` as design-inspectable prove/disprove checks** and prose that implies **artifact boundary**. **Quality bar** rejects hypotheses that are not digital products (remove-the-software test), are wrong-shape for a declared `Target surfaces` value, or whose rationale claims user preference via adjectives instead of a named software-unique mechanism. Rationale must include the switch reason tied to a mechanism (instant lookup, persistence, accumulation, search/recommendation, real-time, async, programmatic integration, computation, automation, network effect). | Inlined by `/api/incubate` inside `<hypotheses_generator_guidance>`. |
| Optional incubation prelude | `incubator-brainstorm-system` / `gen-brainstorm.md` | Divergent brainstorm: 10–15 categorically different product directions from the design brief, anti-censorship framing ("the wilder the better"). Service-shaped seeds are explicitly welcomed at this stage — the product-shape gate downstream transforms or sets aside non-product directions. | Inlined by [`server/lib/incubator-brainstorm.ts`](server/lib/incubator-brainstorm.ts) when `promptOptions.brainstormFirst === true`. |
| Optional incubation prelude | `incubator-curation-system` / `gen-curation.md` | Convergent curation in three parts: (1) **product-shape filter** — every picked direction must be a digital product (remove-the-software test); service / meeting / config seeds are transformed into product seeds or set aside; (2) **surface fit** — when the constraints declare a target surface, picks must fit it; (3) **spread maximization** across digital products. Output is stitched into the design brief as a `<product_shape_candidates>` block; a two-paragraph "Spread rationale" names the transformations and set-asides as an audit trail. | Inlined by [`server/lib/incubator-brainstorm.ts`](server/lib/incubator-brainstorm.ts) immediately after the brainstorm stage when the toggle is set. |
...
| Hypothesis-stage design instructions | `designer-agent-instructions` / [`design-agent-instructions.md`](packages/auto-designer-pi/prompts/design-agent-instructions.md) | Executes the hypothesis: **aligns artifact breadth** with hypothesis + spec (no default one-pager when flow-shaped), sizes MVP for **`measurements`** to be checkable, **quality_bar**, and closes with a **`<final_report>`** instructing the agent to write `BUILD.md` (scope / working features with usage notes / stubbed features) as its last action — surfaced on the canvas's Build tab. | Loaded with the glue via `Promise.all` in `buildHypothesisWorkspaceBundle()`; interpolated as `{{DESIGN_AGENT_INSTRUCTIONS}}`. |
...
| Evaluation | `evaluator-implementation` / `eval-implementation.md` | LLM evaluator rubric for static frontend implementation quality: structure, semantics, responsiveness, JavaScript hygiene, and whether the code expresses the hypothesis. | Loaded by `runEvaluationWorkers()` when evaluation is active. |
...
| `design-quality` / Design quality | Hypothesis-as-thesis frame, design-system precedence rule, and the visual-craft lens (typography, color, spatial composition, motion, atmosphere, content) plus the per-artifact checklist. Every quality choice argues for the hypothesis. | `design` |
