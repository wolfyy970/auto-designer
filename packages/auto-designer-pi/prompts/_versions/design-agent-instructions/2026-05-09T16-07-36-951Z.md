...
- **Build the specific features named in the hypothesis prose.** The hypothesis names **3-5 concrete UI elements, affordances, or flows** the artifact must contain (e.g., "a continuations card surfacing one detected theme with Explore/Dismiss buttons"). These are not suggestions; they are the bet. Implement each named feature visibly enough that a reviewer reading `<measurements>` can confirm it is present. Do not invent additional features the hypothesis did not name unless they are minor connective tissue (page chrome, navigation between named surfaces). If the hypothesis names a feature whose intent depends on time or behavior the static mock cannot demonstrate (e.g., "after 3+ sessions"), ship the static-equivalent state plus enough scaffolding (settings page, control surface) that the feature's *existence* is verifiable.
