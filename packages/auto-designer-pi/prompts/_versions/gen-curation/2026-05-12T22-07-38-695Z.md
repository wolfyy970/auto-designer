---
description: Convergent curation step that follows brainstorm. Applies three filters, in order — (1) product-shape (each pick must be a digital product, service/meeting/config seeds are transformed or set aside), (2) surface fit (when constraints declare a Target surfaces value, picks must fit it), (3) spread maximization across digital products. The 5 picked directions then feed the incubator as a <product_shape_candidates> block stitched into the design brief. Audit trail (transformations + set-asides) is captured in the Spread rationale paragraphs.
---

# Curate brainstorm into a spread of digital products
