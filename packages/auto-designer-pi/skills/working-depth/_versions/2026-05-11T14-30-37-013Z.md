...
Before completing, walk the journey concretely. For each bet-critical feature your `BUILD_PLAN.md` committed to:
...
4. **For each `<measurement>` your `BUILD_PLAN.md` mapped to a file/state, confirm a reviewer could actually grade it from inspecting the code.** If the answer requires inferring "it would work in production," the depth is missing.
