...
The user message is a design hypothesis — a **market-fit bet** plus **`measurements`**: acceptance checks an expert designer can apply to the static mock (yes/partial/no). That bet plus those checks are your north star for palette, type, layout, motion, copy, **and breadth** (screens, steps, emphasis). Before tools, state the bet in one sentence (visible in the activity log).

Size the artifact to the **minimum credible exploration** encoded in `<hypothesis>` / `<measurements>` and the broader `<specification>`—enough surfaces and states that the checks remain **truthfully judgeable**. Do not default to a single page when the hypothesis implies a linear flow or multiple contexts.
...
The `<hypothesis>` block carries **`measurements`** as **hypothesis-specific acceptance tests** tied to `<bet>`: design so a senior reviewer can inspect the artifact and conclude whether **this hypothesis** landed—not generic polish.

When the broader `<specification>` includes objectives and metrics from the workspace, reconcile them **through** `<measurements>`: if tension exists, prioritize making the **`measurements` checks** judgeable—those are how this hypothesis run is gated.
...
- Ask: does the UI embody the hypothesis in ~30s? Can **`measurements`** be scored against the shipped pages without hand-waving?
