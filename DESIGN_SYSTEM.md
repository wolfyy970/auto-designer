...
| **Badge** (`@ds/components/ui/badge`) | `shape="pill"` (rounded-full, border, `font-mono text-nano`) or `shape="tab"` (rounded, no border, `text-badge font-medium`) × `tone="warning|success|accent|neutral"` | Replaces the old inline chip pattern in node components. Migrated sites include input status, hypothesis hints, Incubator hints, and Auto-improve-gated best-result labels. cva variants + compoundVariants split to `badge-variants.ts`. Tests at `components/ui/__tests__/badge.test.tsx`. |
