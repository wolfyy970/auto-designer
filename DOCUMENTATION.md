...
8. **Settings is the model source** — the canvas Model node was removed in Phase 7 D. Each task reads `(providerId, modelId, effort)` from `thinkingDefaultsStore.getEffective(task)`; canvas-migration **v32** strips Model nodes from saved snapshots after the one-shot `migrateModelNodeToSettings()` boot step copies their selections into Settings. Document in **[ARCHITECTURE.md](ARCHITECTURE.md)** only; product copy in **README.md** / **USER_GUIDE.md**.
