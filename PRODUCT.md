...
| Incubator            | Processing | **Incubates** active sources into hypothesis strategies via LLM: filled spec inputs and connected preview references. It assembles those sources deterministically. **Generate** (batch count) and **blank hypothesis** both require a non-empty **Design Brief**; blank adds an empty strategy card without calling the LLM. Optional **Brainstorm directions first** toggle prepends a divergent brainstorm + spread-curation prelude — wider corpus on open-ended briefs, ~50% slower; off by default. After a run, repeated **Generate more** clicks add fresh hypotheses that exclude the existing ones via the prompt's `existingStrategies` anti-repetition block. |
