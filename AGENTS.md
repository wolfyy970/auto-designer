...
Vitest excludes `server/services/__tests__/browser-playwright-evaluator.test.ts` via `vite.config.ts` so the default suite stays hermetic; run **`pnpm test:playwright-eval`** (or `vitest run -c vitest.playwright.config.ts …` on that file) when changing Playwright merge logic. Pi virtual FS tools and the designer extension are covered inside the package: `cd packages/auto-designer-pi && pnpm vitest run`.

**Verification reminder:** the root `pnpm test` does **not** include `packages/auto-designer-pi/__tests__/`. When a change touches the package surface (deleted exports, added/removed prompt files, public API edits), run **both** `pnpm test` **and** `( cd packages/auto-designer-pi && pnpm vitest run )` before committing.
...
**Prompts and skills:** All agent-facing text ships in the [`@auto-designer/pi`](packages/auto-designer-pi/) package. Three real skills (`accessibility`, `design-generation`, `design-quality`) under `packages/auto-designer-pi/skills/<key>/SKILL.md` carry YAML frontmatter + body. Per-task prompt templates (the designer system prompt `_designer-system.md`, plus `gen-hypotheses.md`, `eval-*.md`, `gen-*.md`, `ds-*.md`, `revise.md`, `artifact-conventions.md`) live flat under `packages/auto-designer-pi/prompts/`. Host-side resolution by `PromptKey` is centralised in **[server/lib/prompt-resolution.ts](server/lib/prompt-resolution.ts)**; structural placeholder glue is in **[server/lib/prompt-templates.ts](server/lib/prompt-templates.ts)**. Incubation, inputs-gen, design-system extraction, and the revision round inline the relevant prompt body into the agent user prompt. **Skills use Pi's stock skill flow:** each session seeds the filtered SKILL.md files into the just-bash VFS at `/home/user/project/.skills/<name>/SKILL.md`, Pi's `formatSkillsForPrompt` prints the VFS path in `<location>`, the agent loads each one through the regular `read` tool when the description matches its task. **The Incubator's own context is built deterministically by `buildInternalContext(spec)` in [src/lib/prompts/internal-context.ts](src/lib/prompts/internal-context.ts) — no LLM synthesis step.**

**Upgrading `@mariozechner/pi-coding-agent`.** The Pi tool surface is declared in `packages/auto-designer-pi/src/internal/pi-tool-surface.ts` as a `ToolSurface` registry; `ToolSurface.build()` reads upstream Pi's `allToolNames` literal at session-construction time and throws `ToolSurfaceError` if any built-in lacks a disposition. After bumping the dependency, run `( cd packages/auto-designer-pi && pnpm vitest run )` — the `pi-tool-surface` test fails immediately if Pi added a new tool. For each new tool, add **one** entry to the registry: `{ kind: 'sandboxed-pi', name: '<n>', build: () => <wrapped-tool> }` (sandbox it via the matching Pi factory + `*Operations` injection) **or** `{ kind: 'excluded-pi', name: '<n>', reason: '<why-not>' }` (deny it to the model with documented reasoning). Until you make that decision, sessions will not start — by design. There is no documentation-only promise to remember.

**Version store (committed):** Package skills keep timestamped copies under **`packages/auto-designer-pi/skills/<key>/_versions/`**; package prompt templates (including `_designer-system.md` and `artifact-conventions.md`) snapshot under **`packages/auto-designer-pi/prompts/_versions/<name>/`** (single shared `_versions/` dir since prompts are flat files; Pi's loader does NOT recurse, so the dir is invisible to slash-command discovery); rubric snapshots stay under **`.prompt-versions/snapshots/`**; **`.prompt-versions/manifest.jsonl`** logs everything. Meta-harness **proposer** / **promotion** still use **`snapshotBeforeWrite`** (**[meta-harness/version-store.ts](meta-harness/version-store.ts)**). **Manual workflow:** **`pnpm snap`** (no args) saves only files that changed since the last snapshot; pre-commit runs the same unless **`SKIP_SNAP=1`**. Details: **[USER_GUIDE.md § Version history](USER_GUIDE.md#version-history)**; harness-only: **[meta-harness/VERSIONING.md](meta-harness/VERSIONING.md)**.
