...
**Prompts and skills:** All agent-facing text ships in the [`@auto-designer/pi`](packages/auto-designer-pi/) package. Three real skills (`accessibility`, `design-generation`, `design-quality`) under `packages/auto-designer-pi/skills/<key>/SKILL.md` carry YAML frontmatter + body and may include readable sibling resources (`references/`, `scripts/`, `assets/`, `templates/`); scripts are readable only, not executable. Per-task prompt templates (the designer system prompt `_designer-system.md`, plus `gen-hypotheses.md`, `eval-*.md`, `gen-*.md`, `ds-*.md`, `revise.md`, `artifact-conventions.md`) live flat under `packages/auto-designer-pi/prompts/`. Host-side resolution by `PromptKey` is centralised in **[server/lib/prompt-resolution.ts](server/lib/prompt-resolution.ts)**; structural placeholder glue is in **[server/lib/prompt-templates.ts](server/lib/prompt-templates.ts)**. Incubation, inputs-gen, design-system extraction, and the revision round inline the relevant prompt body into the agent user prompt; design generation lets the agent autonomously call `use_skill` for `accessibility` / `design-generation` / `design-quality`. Skills are **not** copied into the just-bash virtual filesystem. **The Incubator's own context is built deterministically by `buildInternalContext(spec)` in [src/lib/prompts/internal-context.ts](src/lib/prompts/internal-context.ts) — no LLM synthesis step.**
